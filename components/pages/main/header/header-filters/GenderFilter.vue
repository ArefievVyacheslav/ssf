<template lang="pug">
  .filter__item.gender.pos-rel.df.jcsb.aic.cp(@click.stop.prevent="isShowGenderList = !isShowGenderList")
    h3.filter__name Пол
      ...
      DropDown(@close="isShowGenderList = false" :styles="{ padding: '16px 20px 20px 20px', 'min-width': '166px' }")
        ...
        .filter__options-wrapper
          .filter__options-item.df.aic.pl10px.cp(
            v-for="genderObj,idx in genderOptions" :key="idx" @click="toggleGender(genderObj); isShowGenderList = false"
...
export default {
  ...
  methods: {
    ...mapMutations('filters', [ 'SET_FILTER_PARAM', 'SET_FIND_PARAM', 'UNSET_FIND_PARAM' ]),
